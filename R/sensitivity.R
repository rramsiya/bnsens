...
#'@param new_value numeric vector with elements between 0 and 1. Values to which the parameter should be updated. It can take a specific value or more than one. In the case in which more than one value are being defined, the vector should have an increasing order of the elements. \code{new_value} can take also as value the character string \code{all}. If \code{all}, a sequence of possible parameter changes ranging from 0.05 to 0.95 will be considered.
